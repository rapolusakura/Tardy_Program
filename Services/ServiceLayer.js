function setUpSheets(sid, isSigningIn, sheetText) {
  async: false;
  var gs_sid = sid
  ...
  var gs_body = '{"requests":[{"addSheet":{"properties":{"title":"SIGN-IN","tabColor": {"blue":1}}}},{"addSheet":{"properties":{"title":"SIGN-OUT","tabColor":{"red":1.0}}}},{"deleteSheet":{"sheetId":0}}]}';
  ...
  xhr.onload = function() {
    ...
    if (gs_atok) {
      ...
      xxhr.onload = function() {
        if (xxhr.status == 200) {
          // Success
          addRecord(sid, isSigningIn, sheetText);
...
function addRecord(sid, isSigningIn, sheetText) {
  async: false;
  var gs_sid = sid;
  var gs_clid = service.gs_clid; // Enter your API Client ID here
  var gs_clis = service.gs_clis; // Enter your API Client Secret here
  ...
  var gs_body = '{"majorDimension":"ROWS", "values":[[' + sheetText + ']]}';
  // HTTP Request Token Refresh
  var xhr = new XMLHttpRequest();
  xhr.open('POST', 'https://www.googleapis.com/oauth2/v4/token?client_id=' + gs_clid + '&client_secret=' + gs_clis + '&refresh_token=' + gs_rtok + '&grant_type=refresh_token');
  xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
